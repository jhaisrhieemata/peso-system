...
<?php
	...
	include('assets/inc/config.php');
		if(isset($_POST['update_gip']))
		{
			...
			$query="UPDATE mis_gip SET applicant_no=?, gip_compliant=?, type_of_application=?, surname=?, firstname=?, middlename=?, suffix=?, contact=?, civil_status=?, sex=?, date_of_birth=?, age_=?, blood_type=?, app_zone_no=?, app_barangay=?, app_municipality=?, app_province=?, religion=?, citizenship=?, language_dialect=?, father_surname=?, father_firstname=?, father_middlename=?, father_occupation=?, number_of_brother=?, mother_surname=?, mother_firstname=?, mother_middlename=?, mother_occupation=?, number_of_sister=?, pa_zone_no=?, pa_barangay=?, pa_municipality=?, pa_province=?, elementary_school=?, elem_type_of_school=?, elem_school_address=?, elem_year_graduated=?, high_school=?, hs_type_of_school=?, strand=?, hs_year_level=?, hs_lastyear_attended=?, hs_school_address=?, college=?, col_type_of_school=?, course=?, col_school_address=?, col_lastyear_attended=?, pre_strand_course=?, psc_school=?, special_skill=?, income_of_parent=?, date_of_application=?, recieved_by=? WHERE gip_id = ?";
			$stmt = $mysqli->prepare($query);
			$rc=$stmt->bind_param('sssssssssssssssssssssssssssssssssssssssssssssssssssssssi', $applicant_no, $gip_compliant, $type_of_application, $surname, $firstname, $middlename, $suffix, $contact, $civil_status, $sex, $date_of_birth, $age_, $blood_type, $app_zone_no, $app_barangay, $app_municipality, $app_province, $religion, $citizenship, $language_dialect, $father_surname, $father_firstname, $father_middlename, $father_occupation, $number_of_brother, $mother_surname, $mother_firstname, $mother_middlename, $mother_occupation, $number_of_sister, $pa_zone_no, $pa_barangay, $pa_municipality, $pa_province, $elementary_school, $elem_type_of_school, $elem_school_address, $elem_year_graduated, $high_school, $hs_type_of_school, $strand, $hs_year_level, $hs_lastyear_attended, $hs_school_address, $college, $col_type_of_school, $course, $col_school_address, $col_lastyear_attended, $pre_strand_course, $psc_school, $special_skill, $income_of_parent, $date_of_application, $recieved_by, $gip_id);
			$stmt->execute();
			/*
            ...
            <div class="content-page">
                <div class="content">
                    ...
                    <div class="container-fluid">
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="page-title-box">
                                    ...
                                    <h4 class="page-title">Update Manage GIP </h4>
                                </div>
                            </div>
                        </div>     
                        <!-- end page title --> 
                        ...
                        <?php
                            $gip_id=$_GET['gip_id'];
                            $ret="SELECT  * FROM  mis_gip WHERE gip_id=?";
                            $stmt= $mysqli->prepare($ret) ;
                            $stmt->bind_param('i',$gip_id);
                            $stmt->execute() ;//ok
                            $res=$stmt->get_result();
                            //$cnt=1;
                                        ...
                                        <form method="post">
                                               <div class="form-row">
                                                ...
                                                <div class="form-group col-md-4">
                                                    ...
                                                    <select id="inputspescompliant" required="required" name="gip_compliant" class="form-control">
                                                    <option value="">-Select-</option>   
                                                    <option value="No" <?php if ($row->gip_compliant == 'No') echo 'selected'; ?>>No</option>
                                                    <option value="Yes" <?php if ($row->gip_compliant == 'Yes') echo 'selected'; ?>>Yes</option>
                                                    </select>
                                            ...
                                            <div class="form-group col-md-4">
                                                ...
                                                <div class="form-group col-md-4">
                                                    ...
                                                    <input type="date" required="required" value="<?php echo $row->date_of_birth;?>" name="date_of_birth" class="form-control" id="date_of_birth_3" placeholder="Date of Birth">
                                                </div>
                                               
                                                <div class="form-group col-md-4">
                                                         <label for="ageDisplay_3" class="col-form-label">Age</label>
                                                         <input type="text" required="required"  value="<?php echo $row->age_;?>" name="age_" class="form-control" id="ageDisplay_3">
                                                     </div>
